...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: ['Crunch My Butter Wiki', 'Crunch My Butter about', 'Crunch My Butter community wiki', 'Crunch My Butter fan wiki', 'Crunch My Butter Roblox ASMR'],
  };
}
        ...
        <div className="card mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <a href={EXTERNAL_LINKS.roblox} target="_blank" rel="noopener noreferrer" className="card text-center hover:border-[var(--color-accent)] transition-colors">
              ...
              <p className="text-xs text-[var(--color-text-muted)] mt-1">Crunch My Butter on Roblox</p>
            </a>
            <a href={EXTERNAL_LINKS.developer} target="_blank" rel="noopener noreferrer" className="card text-center hover:border-[var(--color-accent)] transition-colors">
              <span className="text-2xl mb-2 block">👥</span>
              <span className="font-bold text-sm">{t('about_developerGroup')}</span>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">EX10 community group</p>
            </a>
          </div>
        </div>
      </section>
