...
import { progressionTracks, cashSources, gameStats, tierColor, cashModeColor, trackRoleColor, TRACK_ICONS, CASH_ICONS } from '@/data/game-data';
...
import { ArrowRight, ChevronRight, Shield, Coins, Sparkles, BookOpen, Crown } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const dynCtx: Record<string, string> = {
    ...
    '__trackCount': String(progressionTracks.length),
    '__cashSourceCount': String(cashSources.length),
    '__statCount': String(gameStats.length),
  };
  ...
  return (
    <div>
      ...
      <JsonLd data={itemListJsonLd(
        progressionTracks.map((track) => ({ name: translate(t, track.nameKey) || track.id, path: '/tier-list' })),
        validLocale
      )} />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_data_tracks')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_data_tracks_title')}</h2>
          </div>
        ...
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_data_tracks_desc')}</p>
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr_1fr] gap-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>Track</span><span>Tier</span><span>Payoff</span><span>When to Buy</span>
          </div>
          {progressionTracks.map((track, idx) => {
            const tc = tierColor(track.tier);
            const Icon = TRACK_ICONS[track.id] || Shield;
            return (
              <Link key={track.id} href="/tier-list" className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">{translate(t, track.nameKey) || track.id}</span>
                </span>
                <span><span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{track.tier}</span></span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, track.payoffKey)}</span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, track.timingKey)}</span>
              </Link>
            );
          })}
        </div>
      ...
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_cashFarming')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_module_cashFarming_title')}</h2>
            </div>
            <Link href="/cash-farming" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_module_cashFarming_desc')}</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
            {cashSources.map((source) => {
              const tc = tierColor(source.tier);
              const mc = cashModeColor(source.modeKey);
              const SrcIcon = CASH_ICONS[source.id] || Coins;
              return (
                <Link key={source.id} href="/cash-farming" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{t('tierList_tierLabel')} {source.tier}</span>
                    <span className="flex items-center gap-2">
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full w-fit" style={{ color: mc, background: `${mc}15` }}>{translate(t, source.modeKey)}</span>
                      <SrcIcon className="w-4 h-4 text-[var(--color-accent)]" />
                    </span>
                  </div>
                  <h3 className="font-bold mb-1 group-hover:text-[var(--color-accent)] transition-colors">
                    {translate(t, source.nameKey) || source.id}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)]">{translate(t, source.descKey)}</p>
                </Link>
              );
            })}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_data_stats')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_data_stats_title')}</h2>
          </div>
          <Link href="/updates" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_data_stats_desc')}</p>
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] gap-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>Metric</span><span>Value</span><span>Context</span>
          </div>
          {gameStats.map((stat, idx) => {
            const badgeColor = tierColor(stat.tier);
            return (
              <Link key={stat.id} href="/updates" className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                <span className="font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">{translate(t, stat.labelKey) || stat.id}</span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, stat.noteKey)}</span>
                <span><span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: badgeColor, background: `${badgeColor}15` }}>{stat.value}</span></span>
              </Link>
            );
          })}
        </div>
      ...
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {['S', 'A', 'B', 'C'].map((tier) => {
              const tierTracks = progressionTracks.filter(track => track.tier === tier);
              ...
              return (
                <Link key={tier} href="/tier-list" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] border border-[var(--color-accent)]/20">{tierTracks.length} {t('home_tier_tracksLabel')}</span>
                  </div>
                  <h3 className="font-bold mb-2 group-hover:text-[var(--color-accent)] transition-colors">
                    {tier === 'S' ? t('home_tier_s') : tier === 'A' ? t('home_tier_a') : tier === 'B' ? t('home_tier_b') : t('home_tier_c')}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)]">
                    {tierTracks.map(track => translate(t, track.nameKey) || track.id).join(', ')}
                  </p>
                </Link>
              );
            })}
